...
export default function ShopProfileScreen({ route, navigation }: any) {
  ...
  const submitPay = async () => {
    const providerId = shop?.owner?._id;
    const amount = parseInt(payAmount, 10);
    if (!providerId) { showAlert('Unavailable', 'This shop cannot receive wallet payments yet.'); return; }
    ...
    try {
      ...
      if (res.ok && res.body?.success) {
        ...
        showAlert('Payment Held in Escrow', 'Your payment is safely held. Release it from your Wallet once the order/service is done.');
      } else {
      ...
      <Modal visible={payOpen} transparent animationType="slide" onRequestClose={() => setPayOpen(false)}>
        <View style={styles.payOverlay}>
          <View style={styles.payCard}>
            ...
            <Text style={styles.paySub}>Funds are held safely in escrow and only released to the shop when you confirm your order/service is done.</Text>
