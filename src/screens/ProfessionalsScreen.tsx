...
export default function ProfessionalsScreen() {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Professionals (nearby)</Text>
      ...
      <FlatList data={results} keyExtractor={(i: any) => i._id || String(Math.random())} renderItem={({ item }) => (
        <View style={styles.item}><Text style={{ fontWeight: '600' }}>{item.name || item.fullName || item.phone}</Text><Text>{item.specialization || ''}</Text></View>
      )} />
    </View>
  );
}
