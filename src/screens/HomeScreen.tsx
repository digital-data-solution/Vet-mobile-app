...
export default function HomeScreen({ navigation }: Props) {
  ...
  // UserTabs uses 'Professionals'. Pick the right name based on role.
  ...
  // VetVerification only exists in ProfessionalTabs, so navigate by tab name.
  const navigateToVetVerification = () => navigation.navigate('VetVerification');

  // Subscription tab exists in Professional, KennelOwner, and ShopOwner tabs.
  const navigateToSubscription = () => navigation.navigate('Subscription');

  // Shops tab exists in all logged-in tab sets.
  const navigateToShops = () => navigation.navigate('Shops');
      ...
      {isLoggedIn && isProfessional && (
        <View style={styles.section}>
          ...
          <View style={styles.grid}>
            {/* VetVerification tab only exists for vet role */}
            {userRole === 'vet' && (
              <ActionCard
                emoji="✅"
                label="Get Verified"
                onPress={navigateToVetVerification}
              />
            )}
            <ActionCard
              emoji="⭐"
            ...
            />
            {/* Network tab only exists for vet role; others don't have it */}
            {userRole === 'vet' && (
              <ActionCard
                emoji="👥"
                label="Network"
